...
│   ├── sidepanel-core.js      # 共享工具 / 消息渲染 / Tab 切换 / 聊天历史
│   ├── sidepanel-chat.js      # 对话逻辑
│   ├── sidepanel-tools.js     # 工具操作（抓取/分析/图片/评论/博主）
│   ├── sidepanel-cart.js      # 分析车管理 / 导出
│   └── sidepanel-settings.js  # 设置 / 账号绑定
├── icons/                     # 扩展图标
├── .gitignore                 # Git 忽略规则
├── LICENSE                    # MIT 开源协议
└── README.md                  # 本文件
...
- **v2.3.0**（2026-06）：
  ...
  - 优化 AI 请求架构为 Adapter 模式（buildRequest / parseResponse）
  - 优化侧边栏代码拆分为 5 个模块（core / chat / tools / cart / settings）
  - 优化公共 CSS 变量提取，统一品牌色管理
  - 修复 Popup 开关可能清空 AI 配置的 Bug
  - 修复 content.css 死代码，分析车按钮改用 CSS 类
  - 修复版本号不一致问题
  - 新增 CSP 安全策略声明
  - 清理仓库旧文件和二进制产物
